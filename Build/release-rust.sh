...
for command_name in cargo codesign dscacheutil ditto go rustup security spctl xcrun; do
...
SUBMISSION_ARCHIVE="${RELEASE_DIR}/FluxBar-${RELEASE_VERSION}-macos-${RELEASE_ARCH}-rust-notarization.zip"
FINAL_ARCHIVE="${RELEASE_DIR}/FluxBar-${RELEASE_VERSION}-macos-${RELEASE_ARCH}-rust.zip"
...
echo "1/8 FluxBar-App (Rust-Kern) bauen"
FLUX_CORE=rust CONFIGURATION=Release GOARCH="${RELEASE_ARCH}" "${SCRIPT_DIR}/build-rust.sh"
...
echo "Rust-Release erstellt: ${FINAL_ARCHIVE}"
